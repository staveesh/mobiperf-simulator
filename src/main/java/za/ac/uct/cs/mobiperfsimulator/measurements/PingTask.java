...
public class PingTask extends MeasurementTask {
  ...
  private MeasurementResult executeJavaPingTask() throws MeasurementError {
    ...
    try {       
      ...
      result = constructResult(rrts, packetLoss, Constants.PING_COUNT_PER_MEASUREMENT, totalPingDelay, PING_METHOD_JAVA, true);
    ...
    if (result == null) {
      ...
      result = constructResult(rrts, 0.0, Constants.PING_COUNT_PER_MEASUREMENT, totalPingDelay, PING_METHOD_JAVA, false);
    }
    return result;
  }
  ...
  private MeasurementResult executeHttpPingTask() throws MeasurementError {
    ...
    try {
      ...
      double packetLoss = 1 - ((double) rrts.size() / (double) Constants.PING_COUNT_PER_MEASUREMENT);
      result = constructResult(rrts, packetLoss, Constants.PING_COUNT_PER_MEASUREMENT, duration, PING_METHOD_HTTP, true);
      dataConsumed += pingTask.packetSizeByte * Constants.PING_COUNT_PER_MEASUREMENT * 2;
    ...
    if (result == null) {
      logger.info("java ping fails");
      logger.error(errorMsg);
      result = constructResult(rrts, 0.0, Constants.PING_COUNT_PER_MEASUREMENT, System.currentTimeMillis() - startTime, PING_METHOD_HTTP, false);
    }
    return result;
  }
