...
public class TCPThroughputTask extends MeasurementTask {
  ...
  private static final Logger logger = LoggerFactory.getLogger(TCPThroughputTask.class);
  ...
  private int MAXPROGRESS = Constants.MAX_PROGRESS_BAR_VALUE;
  ...
  public MeasurementResult call() throws MeasurementError {
    ...
    try {
      ...
      logger.error("Error close the socket for " + desc.type);
      isMeasurementSuccessful = false;
    } catch (InterruptedException e) {
      logger.error("Interrupted captured");
      isMeasurementSuccessful = false;
    }

    MeasurementResult result = new MeasurementResult(
                               BeanUtil.getBean(WebSocketService.class).getDeviceId(),
                               TCPThroughputTask.TYPE,
    ...
    logger.debug("TCP Result sending initiated");
  ...
  private boolean acquireServerConfig() throws MeasurementError, IOException,
    ...
    try {
      ...
      iStream.close();
      tcpSocket.close();
      logger.info("Close server Config socket");
    }
    return result;
  }
  ...
  private void uplink() throws MeasurementError, IOException, InterruptedException {
    logger.info("Start uplink task on " + ((TCPThroughputDesc)measurementDesc).target);
    Socket tcpSocket = null;
    InputStream iStream = null;
    ...
    try {
      ...
      do {
        ...
        if (this.DATA_LIMIT_ON &&
          ...
          logger.info("Detect uplink exceeding limitation " +
      ...
      logger.info("Uplink total data comsumption is " + 
      ...
      if (resultMsgLen > 0) {
        ...
        logger.info("Uplink result from server is " + resultMsgStr);
      ...
      logger.info("Total number of sampling result is " + this.samplingResults.size());
      ...
      oStream.close();
      tcpSocket.close();
      logger.info("Close uplink socket");
    }
  }
  ...
  private void downlink() throws MeasurementError, IOException {
    logger.info("Start downlink task on " + ((TCPThroughputDesc)measurementDesc).target);
    Socket tcpSocket = null;
    InputStream iStream = null;
    try {
      tcpSocket = new Socket();
      ...
      logger.error("Downlink socket opening error" + i.getCause().toString());
    ...
    try {
      ...
      do {
        ...
        if (this.DATA_LIMIT_ON &&
          ...
          logger.info("Detect downlink data limitation exceed with " +
      ...
      logger.info("Total download data is " + (double)this.totalRevSize/(1024*1024) + " MB");
      logger.info("Total number of sampling result is " + this.samplingResults.size());
      ...
      iStream.close();
      tcpSocket.close();
      logger.info("Close downlink socket");
    }
  }
