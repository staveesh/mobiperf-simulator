...
public abstract class MeasurementDesc {
  ...
  protected MeasurementDesc(String type, String key, Date startTime, 
    ...
    if (endTime == null || 
        endTime.getTime() - now > Constants.TASK_EXPIRATION_MSEC) {
      this.endTime = new Date(now + Constants.TASK_EXPIRATION_MSEC);
    } else {
      this.endTime = endTime;
    }
    if (intervalSec <= 0) {
      this.intervalSec = Constants.DEFAULT_SYSTEM_MEASUREMENT_INTERVAL_SEC;
    } else {
